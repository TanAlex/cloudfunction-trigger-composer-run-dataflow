...
def move_to_completion_bucket(target_bucket, target_infix, **kwargs):
    ...
    logging.info(dag_run_conf)
    if 'bucket' in dag_run_conf and 'name' in dag_run_conf:
        source_bucket = kwargs['dag_run'].conf['bucket']
        source_object = kwargs['dag_run'].conf['name']
    else:
        logging.error("You didnt trigger this with dag_run.conf['bucket'] and ['name']")
        raise Exception("Not triggered properly")
...
with models.DAG(dag_id='GcsToBigQueryTriggered',
    ...
    # dataflow_task.
    start_task >> dataflow_task >> success_move_task
